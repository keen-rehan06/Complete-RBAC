...
export const registerUser = async(req,res) => {
    try {
        ...
        return res.status(201).send({message:"User regesitered Successfully!!",user}) 
    } catch (error) {
...
export const LoginUser = async(req,res) => {
    try {
        ...
        res.status(200).cookie("token",token).send({message:"Login Sucessfully!",user})
    } catch (error) {
...
export const getAllUser = async(req,res) => {
    ...
    res.status(200).send({message:`Welcome,Admin ${user.name}`,success:true})
}
